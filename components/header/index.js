...
export default function Header () {
    ...
    return (
        <>
            <div className={`${style('header')} fixed w-full h-[80px] top-0 z-[999] flex items-center px-[20px] justify-between 
                lg:h-[100px] lg:px-[100px]
                ${scrollTop > lastScrollTop ? style('hidden-header') : ''}`}>
                <Link passHref href="/">
                    <img src={Logo.src} height="48" className={style('img-logo')} />
                </Link>
                <img onClick={_ => setOpentModal(true)} className="menu-items cursor-pointer lg:hidden" src={MenuItems.src} width="26px" height="26px" /> 
                <ul className={'flex justify-start grow' + style('nav')}>
                    <li className={router.pathname == "/" ? "font-bold" : ""}>
                        <Link href="/">Trang Chủ</Link>
                    </li>
                    <li>
                        <Link href="https://exchange.viref.net">Giao dịch</Link>
                    </li>
