...
export default function ChooseLanguage() {
  const ref = useRef();
  const [language, setLanguage] = useState({name: 'Tiếng Việt', img: VietNamFlag});
  const [showUl, setShowUl] = useState(false);
  useOnClickOutside(ref, () => setShowUl(false));
  const router = useRouter()
  const handleLocaleChange = (value) => {
    if (value === 'en')
      setLanguage({name: 'English', img: EnglishFlag})
    if (value === 'vi')
      setLanguage({name: 'Tiếng Việt', img: VietNamFlag})
